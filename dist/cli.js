...
function parseArgs(argv) {
    ...
    if (command !== "migrate") {
        throw new Error("Usage: doublesigma migrate [path] [--url <https://github.com/...>] [--ref <branch|tag|sha>] [--dry-run|--apply] [--quantum] [--ai] [--keep-temp]");
    }
    let targetPath = ".";
    let repoUrl;
    let ref;
...
async function main() {
    try {
        ...
        if (options.repoUrl) {
            ...
            const payload = await migrateFromGithubHttpsUrl(options.repoUrl, options.ref, {
                mode: options.mode,
