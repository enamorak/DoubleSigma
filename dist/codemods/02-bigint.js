...
export const BIGINT_MIGRATION_RULES = [
    {
        ...
        now: "12345n",
        docsUrl: DOCS,
        apply: (source, fp) => transformSource(source, "ethers.BigNumber.from($S)", (m) => bigNumberFromStringReplacer(m, (inner, q) => (q === '"' && isDecimalDigits(inner) ? `${inner}n` : null)), fp).code,
    },
    {
    ...
    },
    {
        ...
        now: 'BigInt("0x…")',
        docsUrl: DOCS,
        apply: (source, fp) => transformSource(source, "ethers.BigNumber.from($S)", (m) => bigNumberFromStringReplacer(m, (inner, q) => q === '"' && isHexStringBody(inner) ? `BigInt("${inner}")` : null), fp).code,
    },
    {
