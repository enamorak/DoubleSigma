...
import type { QuantumInsights } from "../types.js";
 ...
 */
export async function scanQuantumMarkers(root: string): Promise<QuantumInsights> {
  ...
  const superpositionCoverage =
    burden === 0 ? 1 : Math.min(1, files.length / (burden + files.length));
