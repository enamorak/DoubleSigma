 ...
 */
export async function resolveTarget(targetPath, checkExists) {
 ...
 */
export async function runMigrationJob(options) {
    ...
    const opts = { ...options, targetPath: target.absolute };
    const result = opts.mode === "apply" ? await runApplyMigration(opts.targetPath) : await runDryMigration(opts.targetPath);
    const orderedChanges = opts.quantum && result.changes.length > 0
        ? rankChangesByAmplitude(result.changes)
        : result.changes;
    let resultForLog = { ...result, changes: orderedChanges };
    if (opts.quantum) {
    ...
    if (opts.quantum) {
        if (resultForLog.quantumInsights) {
            ...
            lines.push(`quantum: superposition coverage (heuristic)=${resultForLog.quantumInsights.superpositionCoverage}`);
        }
