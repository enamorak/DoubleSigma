    ...
    return [
        {
            ...
            docsUrl: DOCS,
            confidence: "medium",
            apply: (source, fp) => transformSource(source, fp, (root) => {
                const edits = [];
...
function editsForBigNumberFromStrings(root, pred) {
    const edits = [];
...
export const BIGINT_MIGRATION_RULES = [
    {
        ...
        docsUrl: DOCS,
        apply: (source, fp) => transformSource(source, fp, (root) => {
            const edits = editsForBigNumberFromStrings(root, (inner, q) => q === '"' && /^\d+$/.test(inner) ? `${inner}n` : null);
            if (edits.length === 0)
                return null;
            return root.commitEdits(edits);
        }),
    ...
    },
    {
        ...
        docsUrl: DOCS,
        apply: (source, fp) => transformSource(source, fp, (root) => {
            const edits = editsForBigNumberFromStrings(root, (inner, q) => q === '"' && /^0x[0-9a-fA-F]+$/.test(inner) ? `BigInt("${inner}")` : null);
            if (edits.length === 0)
                return null;
            return root.commitEdits(edits);
        }),
